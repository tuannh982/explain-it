...
export class ChatRetryHandler {
    ...
    async executeWithRetry<T>(
        ...
        for (let attempt = 1; attempt <= opts.maxRetries; attempt++) {
            try {
                ...
                logger.warn(
                    `[${context.agentName}] Attempt ${attempt}/${opts.maxRetries} failed: ${error.message}`
                );
                ...
                await this.logFailure({
                    ...
                    rawContentPreview: error.rawContent?.substring(0, 500)
                }, opts.logFile);

                if (attempt < opts.maxRetries) {
                    const delay = opts.backoffMs * Math.pow(2, attempt - 1);
    ...
    private async logFailure(entry: FailureLogEntry, logFile: string): Promise<void> {
        try {
