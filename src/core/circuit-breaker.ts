...
export class CircuitBreaker {
	...
	checkValidationFailure(): "CONTINUE" | "REDECOMPOSE" | "ABORT" {
		if (
			...
			config.circuitBreaker.maxValidationAttempts
		) {
			return "CONTINUE";
		}
	...
	checkExplanationIteration(conceptId: string): "CONTINUE" | "SKIP_CONCEPT" {
		...
		logger.warn(
			`CircuitBreaker: Max iterations reached for concept ${conceptId}. Skipping.`,
		);
		return "SKIP_CONCEPT";
	}
	...
	checkRedecompositionLimit(): "CONTINUE" | "PARTIAL_OUTPUT" {
		if (
			this.stats.redecompositionCount <
			config.circuitBreaker.maxRedecompositions
		) {
			return "CONTINUE";
		}
		logger.warn(
			"CircuitBreaker: Max redecompositions reached. Proceeding with partial output.",
		);
	...
	checkConceptFailures(totalConcepts: number): "CONTINUE" | "ABORT" {
		...
		if (failureRate > config.circuitBreaker.maxConceptFailures) {
			logger.error(
				`CircuitBreaker: Concept failure rate ${failureRate} exceeds limit.`,
			);
			return "ABORT";
		}
		return "CONTINUE";
