...
import { Message } from '../core/llm/client.js';
...
export class ClarifierAgent extends BaseAgent {
    async execute(input: ClarifierInput): Promise<ClarifierResult> {
        ...
        const { system, user } = await this.templateRenderer.renderParts('clarifier', {
        ...
        if (input.history && input.history.length > 0) {
            if (input.history.length <= MAX_HISTORY_TURNS) {
                ...
                const summaryText = input.history.map(
                    (h, i) => `[Turn ${i + 1}] Q: ${h.question} | A: ${h.answer}`
                ).join('\n');
